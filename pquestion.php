<?php 
    ...
    $query ="SELECT * FROM phpq where question_number= $number"; //compare table qno from $number

    $result = mysqli_query($con,$query);
    $question = mysqli_fetch_assoc($result); //this function convert stored data into associative array
    
    //get options
    $query = "SELECT * FROM phpo where question_number = $number";//all coices fetch from table
    $choices = mysqli_query($con,$query);

    //total questions
    $query ="SELECT * FROM phpq";
    ...
    <body>
        ...
        <main>
                <div class="container">
                    ...
                    <form method="POST" action="pprocess.php">
                        <ul class="choicess">
                        <?php
                        while($row=mysqli_fetch_assoc($choices)){ ?>
                        <li> <input type="radio" name="choice1" value="<?php echo $row['id']; ?>"> <?php echo $row['phpoption'];?></li>
                        <?php } ?>
                        </ul>
